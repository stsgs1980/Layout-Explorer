...
function CategoryBadge({ label, tokens }: { label: string; tokens: ThemeTokens }) {
  return (
...
function RegionsLegend({ recipe, tokens }: { recipe: LayoutRecipe; tokens: ThemeTokens }) {
  return (
...
function ScoreFooter({ recipe, score, tokens }: { recipe: LayoutRecipe; score: number; tokens: ThemeTokens }) {
  return (
    <div style={{ padding: `${spacing.md}px ${spacing.xl}px`, borderTop: `1px solid ${tokens.borderSubtle}`, background: tokens.bgBase, transition: 'background 0.3s' }}>
...
function ViewportSwitcher({ viewport, onViewportChange, tokens }: { viewport: string; onViewportChange: (v: 'mobile' | 'tablet' | 'desktop') => void; tokens: ThemeTokens }) {
  const VIEWPORTS = [
